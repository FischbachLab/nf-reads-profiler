...
sed -i 's/clade_name/sample_name/g' merged_metaphlan_abundance_species.tsv

# output results with full rank taxonomy 
grep -E "s__|metaphlan|UNCLASSIFIED" merged_abundance_table.tsv | grep -v "t__" | sed "s/_metaphlan_bugs_list//g" |  sed "s/k__//g" |  sed "s/p__//g" | sed "s/c__//g" |  sed "s/o__//g" |  sed "s/f__//g" |  sed "s/g__//g" | sed "s/s__//g" | sed "s/|/,/g" \
> full_taxonomy_merged_metaphlan_abundance_species.tsv
sed -i 's/clade_name/full_taxonomy/g' full_taxonomy_merged_metaphlan_abundance_species.tsv
